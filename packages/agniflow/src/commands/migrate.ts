...
export const migrateOptionsSchema = z.object({
  ...
  migration: z
    ...
    .refine(
      ...
      {
        message:
          "You must specify a valid migration. Run `agniflow migrate --list` to see available migrations.",
      }
    )
    .optional(),
})
  ...
  .action(async (migration, opts) => {
    try {
      ...
      if (!options.migration) {
        throw new Error(
          "You must specify a migration. Run `agniflow migrate --list` to see available migrations."
        )
      }
