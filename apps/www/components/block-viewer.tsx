...
import { registryItemFileSchema, registryItemSchema } from "agniflow/registry"
import { z } from "zod"
      ...
      <div className="ml-auto hidden items-center gap-2 md:flex">
        ...
        <div className="flex h-7 items-center gap-1 rounded-md border p-[2px]">
          <Button
            ...
            size="sm"
            onClick={() => {
              copyToClipboard(`npx agniflow@latest add ${item.name}`)
            }}
          >
            {isCopied ? <Check /> : <Terminal />}
            <span className="hidden lg:inline">npx agniflow add {item.name}</span>
          </Button>
        </div>
